...
function UnitIcon({ squad, selected, onSelect }) {
  ...
  return (
    ...
    >
      ...
      <span className="mt-0.5 font-mono text-[10px] leading-none text-slate-100">
        {squad.alive}/{squad.soldiers}
      </span>
      <span
        className="absolute bottom-0 left-0 h-0.5 rounded bg-current"
...
function UnitStrip({ title, squads, selectedId, onSelect, tone }) {
  return (
    <div className="min-w-0 flex-1">
      <div
        className={`mb-1 text-[11px] font-bold uppercase tracking-[0.18em] ${tone}`}
      >
        {title}
      </div>
        ...
        {squads.length ? (
          squads.map((squad) => (
            <UnitIcon
              key={squad.id}
              squad={squad}
              selected={selectedId === squad.id}
              onSelect={onSelect}
            />
          ))
        ) : (
          <div className="flex items-center px-3 text-[11px] text-slate-500">
            Deploy units
          </div>
        )}
      </div>
...
export function UnitCommandCard({
  ...
  const resolvedPosition = position ?? screenPosition;
  const issueCommand = onCommand ?? onStartCommand;
  if (!squad || !resolvedPosition) return null;

  const isFriendly = squad.side === "friendly";
  const strength = Number.isFinite(squad.strengthPct) ? squad.strengthPct : 0;
  const left = Math.max(
    16,
    Math.min(window.innerWidth - 292, resolvedPosition.x - 136),
  );
  const top = Math.max(
    84,
    Math.min(window.innerHeight - 300, resolvedPosition.y - 210),
  );
    ...
    >
      <div className="mb-2 flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-0.5 text-slate-400">
            Status: <b className="text-slate-100">{squad.state}</b>
          </div>
        </div>
        <button
          className="rounded px-2 text-lg leading-none text-slate-400 hover:bg-slate-800 hover:text-white"
          onClick={onClose}
        >
          ×
        </button>
      ...
      {commandMode ? (
        ...
        <div className="grid grid-cols-3 gap-2">
          <button
            className="rounded border border-slate-700 bg-slate-900 px-2 py-2 hover:bg-sky-800"
            onClick={() => issueCommand?.("move")}
          >
            Move
          </button>
          <button
            className="rounded border border-slate-700 bg-slate-900 px-2 py-2 hover:bg-emerald-800"
            onClick={() => issueCommand?.("defend")}
          >
            Defend
          </button>
          <button
            className="rounded border border-slate-700 bg-slate-900 px-2 py-2 hover:bg-rose-800"
            onClick={() => issueCommand?.("suppress")}
          >
            Suppress
          </button>
          <button
            className="rounded border border-slate-700 bg-slate-900 px-2 py-2 hover:bg-orange-800"
            onClick={onRetreat}
          >
            Retreat
          </button>
          <button
            className="col-span-2 rounded border border-slate-700 bg-slate-900 px-2 py-2 hover:bg-amber-800"
            onClick={onHold}
          >
            Hold
          </button>
...
export function UnitDashboard({ infantryStats, squads, onSelectSquad }) {
  const resolvedSquads = infantryStats?.squads ?? squads ?? [];
  const selectedId = infantryStats?.selectedSquadId ?? null;
  const friendly = resolvedSquads.filter((squad) => squad.side === "friendly");
  const enemy = resolvedSquads.filter((squad) => squad.side === "enemy");
